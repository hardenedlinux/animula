...
GC_INLINE_STATIC GC_word GC_FASTCALL
GC_stack_approx_size (CONST struct GC_gcdata_s *gcdata)
{
...
GC_guess_collect (CONST struct GC_gcdata_s *gcdata, GC_word objsize)
{
...
GC_INLINE_STATIC GC_word GC_FASTCALL
GC_guess_expand_size (CONST struct GC_gcdata_s *gcdata, GC_word objsize)
{
  GC_word space_divisor = GC_free_space_divisor + 1;
  return (gcdata->marked_bytes + gcdata->bytes_allocd) / space_divisor
             >= gcdata->free_bytes
           ? gcdata->free_bytes * space_divisor + (gcdata->bytes_allocd >> 3)
               + (objsize << 2) + gcdata->dataroot_size
           : 0;
}
      ...
      if ((max_heapsize = GC_max_heapsize) > total_heapsize)
        {
          ...
          if (GC_verbose_gc)
            fprintf (
              stdout,
              "[GC: Expand by %lu KiB after %lu KiB allocd, %lu KiB free of "
              "%lu KiB]" GC_NEW_LINE,
      ...
      if (dataroot->begin_addr < begin_addr)
        {
          if (end_addr < dataroot->end_addr)
            {
              new_dataroot = GC_core_malloc_with_gc (
                gcdata, sizeof (struct GC_dataroot_s), &res);
              res = -1;
                                        ...
                                        GC_word begin_addr, GC_word end_addr)
{
  ...
  if (begin_addr)
    {
      begin_addr
        = (begin_addr + (sizeof (GC_word) - 1)) & ~(sizeof (GC_word) - 1);
                  ...
                  while (dataroot != NULL && end_addr >= dataroot->begin_addr)
                    {
                      if (dataroot->end_addr >= end_addr)
                        end_addr = dataroot->end_addr;
                      gcdata->dataroot_size
                        -= dataroot->end_addr - dataroot->begin_addr;
  ...
  if ((gcdata = GC_gcdata_global) == NULL)
  ...
  else
    {
      ...
      if (cur_stack == NULL || cur_stack->thread_id != thread_id)
        {
          cur_stack = gcdata->stkroot_htable.hroots[GC_HASH_INDEX (
          ...
          if ((gcdata->cur_stack = cur_stack) == NULL)
            {
              if (GC_HASH_RESIZECOND (gcdata->stkroot_htable.count,
                                      gcdata->stkroot_htable.log2_size)
                  && (new_hroots = GC_alloc_hroots (
                        gcdata,
                        new_log2_size = gcdata->stkroot_htable.log2_size + 1,
                        NULL))
                       != NULL)
                GC_stkroot_tblresize (gcdata, new_hroots, new_log2_size);
              res = 0;
              if ((cur_stack = GC_core_malloc_with_gc (
...
GC_set_inactive_sp (struct GC_gcdata_s **pgcdata)
{
  ...
  if (GC_stack_grows_up)
    ...
    cur_stack->begin_addr = (GC_word)pgcdata + (sizeof (GC_word) << 1);
  cur_stack->inactive = 1;
  return 1;
}

GC_INLINE_STATIC int GC_FASTCALL
GC_set_activation_frame (struct GC_activation_frame_s *activation_frame,
                         struct GC_stkroot_s *cur_stack)
{
  if (cur_stack == NULL || !cur_stack->inactive)
    return 0;
  activation_frame->inactive_sp
    = GC_stack_grows_up ? cur_stack->end_addr : cur_stack->begin_addr;
...
GC_restore_inactive_sp (struct GC_stkroot_s *cur_stack,
                        CONST struct GC_activation_frame_s *activation_frame)
{
          ...
          if (((addr = *region) & align_mask) == 0)
            {
              ...
              if ((GC_word) (objlink
                ...
                for (;;)
                  {
                    if ((GC_word) (objlink = *(pnext = &objlink->next))->obj
                        <= addr)
                      break;
                  }
              if (interior_pointers)
                {
                  if ((objlink->atomic_and_size
                       & ~(GC_ATOMIC_MASK | GC_HASDSLEN_MASK))
                      <= addr - (GC_word)objlink->obj)
                    {
                      if ((addr
                        ...
                        continue;
                      pnext
                        = &hroots[((((addr - ((GC_word)1 << GC_LOG2_OFFIGNORE))
                                     >> log2_size)
                                    ^ (addr
      ...
      if (GC_stack_grows_up)
          ...
          do
            {
              GC_scan_region (gcdata,
                              ((GC_word)activation_frame
                               + (sizeof (struct GC_activation_frame_s)
                                  + (sizeof (GC_word) << 1) - 1))
                                & ~(sizeof (GC_word) - 1),
      ...
      else
        {
          do
            {
              GC_scan_region (gcdata,
                              (begin_addr + (sizeof (GC_word) - 1))
                                & ~(sizeof (GC_word) - 1),
                              ((GC_word)activation_frame - sizeof (GC_word))
                                & ~(sizeof (GC_word) - 1),
                              1);
...
#    endif
                    {
                      ...
                      if (GetThreadContext (GC_THREAD_HANDLE (stkroot),
                        ...
                        {
                          GC_scan_region (
                            gcdata, (GC_word) (&context),
                            (GC_word) (&context)
                              + (sizeof (context) & ~(sizeof (GC_word) - 1)),
                            1);
                          if ((*stop_func) ())
                            break;
                          *(GC_stack_grows_up ? &stkroot->end_addr
                                              : &stkroot->begin_addr)
                      ...
                      struct GC_stkroot_s **new_hroots, GC_word new_log2_size)
{
  ...
  gcdata->free_bytes
    += ((GC_word)sizeof (GC_word) << gcdata->stkroot_htable.log2_size)
       + sizeof (GC_word);
...
GC_INLINE_STATIC void GC_FASTCALL
GC_stkroot_delete_cur (struct GC_gcdata_s *gcdata)
{
  struct GC_stkroot_s *cur_stack = gcdata->cur_stack;
  GC_REGISTER_KEYWORD struct GC_stkroot_s **pnext
    = &gcdata->stkroot_htable.hroots[GC_HASH_INDEX (
      (GC_word)cur_stack->thread_id, gcdata->stkroot_htable.seed,
      gcdata->stkroot_htable.log2_size)];
...
GC_STATIC void GC_FASTCALL GC_dlink_scan_clear (struct GC_gcdata_s *gcdata)
{
  ...
  GC_word count = 0;
  obj_hmask
    = ((GC_word)1 << (obj_log2_size = gcdata->obj_htable.log2_size)) - 1;
  for (;;)
    {
      for (;;)
        {
          if (*(struct GC_dlink_s **)(addr += sizeof (GC_word))
              != &GC_nil_dlink)
            break;
        }
...
GC_STATIC GC_word GC_FASTCALL GC_dlink_free_pending (struct GC_gcdata_s *gcdata,
                                                     GC_word min_free_count)
{
  GC_REGISTER_KEYWORD struct GC_dlink_s *dlink;
  GC_word count = 0;
  do
...
GC_INLINE_STATIC void GC_FASTCALL
GC_dlink_tblresize (struct GC_gcdata_s *gcdata, struct GC_dlink_s **new_hroots,
                    GC_word new_log2_size)
{
  GC_REGISTER_KEYWORD GC_word addr
    = (GC_word)gcdata->dlink_htable.hroots - sizeof (GC_word);
                                               ...
                                               struct GC_dlink_s *new_dlink)
{
  ...
  pnext = &gcdata->dlink_htable.hroots[GC_HASH_INDEX (
    hidden_link >> GC_LOG2_OFFIGNORE, gcdata->dlink_htable.seed,
...
{
  GC_REGISTER_KEYWORD struct GC_dlink_s *dlink;
  struct GC_dlink_s **pnext = &gcdata->dlink_htable.hroots[GC_HASH_INDEX (
    hidden_link >> GC_LOG2_OFFIGNORE, gcdata->dlink_htable.seed,
    gcdata->dlink_htable.log2_size)];
      ...
      if ((objlink->atomic_and_size & ~(GC_ATOMIC_MASK | GC_HASDSLEN_MASK))
          <= addr - (GC_word)objlink->obj)
        {
          ...
          pnext = &hroots[((((addr - ((GC_word)1 << GC_LOG2_OFFIGNORE))
                             >> log2_size)
                            ^ (addr - ((GC_word)1 << GC_LOG2_OFFIGNORE)))
                           >> GC_LOG2_OFFIGNORE)
                          & (((GC_word)1 << log2_size) - 1)];
          while ((GC_word) (objlink = *pnext)->obj > addr)
            pnext = &objlink->next;
...
GC_INLINE_STATIC GC_word GC_FASTCALL
GC_fnlz_precollect (struct GC_gcdata_s *gcdata, GC_word *pcount)
{
  GC_REGISTER_KEYWORD GC_word addr;
      ...
      do
        {
          ...
          if ((client_data = fnlz->client_data) != NULL)
            (void)GC_objlink_mark (gcdata, (GC_word)client_data,
                                   interior_pointers);
        }
      ...
      for (;;)
        {
          for (;;)
            {
              if (*(struct GC_fnlz_s **)(addr += sizeof (GC_word))
                  != &GC_nil_fnlz)
                break;
            }
...
GC_STATIC GC_word GC_FASTCALL GC_fnlz_after_collect (struct GC_gcdata_s *gcdata,
                                                     GC_word *pcount)
{
  GC_REGISTER_KEYWORD GC_word addr
  ...
  for (;;)
    {
      ...
      do
        {
          if (GC_objlink_mark (gcdata, (GC_word) (objlink = fnlz->objlink)->obj,
                               0))
            {
              *pnext = fnlz->next;
...
GC_INLINE_STATIC void GC_FASTCALL
GC_fnlz_tblresize (struct GC_gcdata_s *gcdata, struct GC_fnlz_s **new_hroots,
                   GC_word new_log2_size)
{
  GC_REGISTER_KEYWORD GC_word addr
    = (GC_word)gcdata->fnlz_htable.hroots - sizeof (GC_word);
      ...
      do
        {
          ...
          pnext = &new_hroots[GC_HASH_INDEX (
            (GC_word)objlink / (sizeof (GC_word) << 1), seed, new_log2_size)];
          while (
            (GC_word) ((struct GC_fnlz_s *)(addr = (GC_word) (*pnext)))->objlink
            > (GC_word)objlink)
  ...
  gcdata->free_bytes
    += ((GC_word)sizeof (GC_word) << gcdata->fnlz_htable.log2_size)
       + sizeof (GC_word);
  gcdata->fnlz_htable.hroots = new_hroots;
  gcdata->fnlz_htable.log2_size = new_log2_size;
}

GC_INLINE_STATIC GC_finalization_proc GC_FASTCALL
GC_fnlz_add_del (struct GC_gcdata_s *gcdata, struct GC_objlink_s *objlink,
                 GC_finalization_proc fn, void *client_data, void **odata)
{
  ...
  pnext = &gcdata->fnlz_htable.hroots[GC_HASH_INDEX (
    (GC_word)objlink / (sizeof (GC_word) << 1), gcdata->fnlz_htable.seed,
      ...
      else
        {
          *pnext = fnlz->next;
          if (gcdata->fnlz_htable.single_free != NULL)
            ...
            gcdata->fnlz_htable.single_free = fnlz;
          if (!(--gcdata->fnlz_htable.count))
            gcdata->fnlz_htable.has_client_ptrs = 0;
        }
    }
...
#  endif
  ...
  if ((GC_word) (
        ...
        = *(pnext
            = &gcdata->obj_htable
                 .hroots[(((((GC_word)obj) >> gcdata->obj_htable.log2_size)
                           ^ (GC_word)obj)
                          >> GC_LOG2_OFFIGNORE)
                         & (((GC_word)1 << gcdata->obj_htable.log2_size) - 1)]))
        ->obj
      > (GC_word)obj)
    for (;;)
      {
...
GC_STATIC GC_word GC_FASTCALL GC_objlink_remove_all (struct GC_gcdata_s *gcdata)
{
  GC_REGISTER_KEYWORD GC_word addr
  ...
  for (;;)
    {
      for (;;)
        {
          if (*(struct GC_objlink_s **)(addr += sizeof (GC_word))
              != &GC_nil_objlink)
            break;
        }
          ...
          do
            {
              ...
              if (max_obj_addr <= addr)
                max_obj_addr = addr;
              pnext
                = &hroots[(((addr >> log2_size) ^ addr) >> GC_LOG2_OFFIGNORE)
                          & hmask];
              while ((GC_word) (objlink = *pnext)->obj > addr)
                pnext = &objlink->next;
              count++;
              if ((GC_word)displaced_pointer - addr
                  < (marked_list->atomic_and_size
                     & ~(GC_ATOMIC_MASK | GC_HASDSLEN_MASK)))
                break;
      ...
      if (marked_list != NULL)
        {
          if (gcdata->obj_htable.follow_list != NULL)
            ...
            gcdata->obj_htable.marked_list = marked_list->next;
          *pnext = marked_list;
...
GC_STATIC GC_word GC_FASTCALL
GC_objlink_some_refill (struct GC_gcdata_s *gcdata, GC_word max_count)
{
  GC_REGISTER_KEYWORD struct GC_objlink_s *marked_list;
  GC_REGISTER_KEYWORD struct GC_objlink_s **pnext;
  struct GC_objlink_s *objlink;
  ...
  while ((objlink = gcdata->obj_htable.free_list) != NULL)
    {
      ...
      if (gcdata->dlink_htable.count)
        {
          min_hidden = GC_HIDE_POINTER (
            (char *)objlink->obj
            + (objsize
               & ~(GC_ATOMIC_MASK | GC_HASDSLEN_MASK | (sizeof (GC_word) - 1)))
            - (GC_word)sizeof (GC_word));
          if ((max_hidden = GC_HIDE_POINTER (objlink->obj)) >= min_hidden)
            {
              (void)GC_dlink_delete (gcdata, max_hidden, min_hidden,
                                     max_hidden);
              if (((min_hidden ^ max_hidden)
                   & ~(((GC_word)1 << GC_LOG2_OFFIGNORE) - 1))
                  != 0)
                (void)GC_dlink_delete (
                  gcdata, max_hidden - ((GC_word)1 << GC_LOG2_OFFIGNORE),
      ...
      if ((objsize & GC_ATOMIC_MASK) == 0)
        gcdata->followscan_size
          -= objsize & ~(GC_HASDSLEN_MASK | (sizeof (GC_word) - 1));
      objlink->obj = NULL;
      objlink->next = gcdata->obj_htable.unlinked_list;
      gcdata->obj_htable.unlinked_list = objlink;
      totalsize += objsize;
      count++;
      if ((gcdata->free_bytes += objsize & ~(GC_ATOMIC_MASK | GC_HASDSLEN_MASK))
          >= min_free_bytes)
        break;
    }
  ...
  if (gcdata->dataroots != NULL)
    {
      ...
      if (GC_verbose_gc)
        {
          fprintf (stdout,
                   "[GC: #%lu Scan %lu KiB after %lu KiB allocd, %lu KiB free "
                   "of %lu KiB]" GC_NEW_LINE,
                   (unsigned long)(GC_gc_no + 1),
                   GC_SIZE_TO_ULKB (GC_stack_approx_size (gcdata)
                                    + gcdata->dataroot_size
                                    + gcdata->followscan_size),
                   GC_SIZE_TO_ULKB (gcdata->bytes_allocd),
              ...
              if (!stopped)
                {
                  ...
                  if ((oldcnt >> 1) < obj_count
                      && GC_HASH_RESIZECOND (oldcnt,
                                             gcdata->obj_htable.log2_size)
                      && (new_hroots = GC_alloc_hroots (
                            gcdata, gcdata->obj_htable.log2_size + 1,
                            &GC_nil_objlink))
                           != NULL)
                    {
      ...
      if (!gcdata->bytes_allocd)
        {
          ...
          if (GC_verbose_gc)
            {
              fprintf (stdout,
                       "[GC: Done in %lu ms, %lu KiB used by %lu objs, %lu KiB "
                       "used by GC]" GC_NEW_LINE,
                       GC_CURTIME_GETMS (&curt) - time_ms,
                       GC_SIZE_TO_ULKB (gcdata->marked_bytes),
...
#  ifndef GC_NO_DLINKS
              if (dlinks_count)
                fprintf (
                  stdout,
                  "[GC: %lu disappearing links cleared of %lu "
                                                  ...
                                                  GC_word size, int dont_expand)
{
  ...
  if (size <= (~(GC_ATOMIC_MASK | GC_HASDSLEN_MASK) < GC_MEM_SIZELIMIT
                 ? ~(GC_ATOMIC_MASK | GC_HASDSLEN_MASK)
                 : GC_MEM_SIZELIMIT)
      && ((free_bytes = gcdata->free_bytes) > size
          || (!dont_expand
              && gcdata->total_heapsize - (free_bytes - size)
                   <= GC_max_heapsize)))
    {
      do
        {
      ...
      if (ptr != NULL)
        {
          if (free_bytes < size)
            ...
            gcdata->free_bytes = free_bytes - size;
        }
    }
  return ptr;
}
  ...
  if ((ptr
      ...
      || ((
            ...
            GC_dlink_free_pending (gcdata, ~(GC_word)0) +
              ...
              != 0
            || GC_dont_expand)
          && (ptr = GC_inner_core_malloc (gcdata, size + sizeof (GC_word), 0))
               != NULL))
    {
  ...
  if ((ptr = GC_inner_core_malloc (gcdata, size, GC_dont_expand)) == NULL)
    {
      ...
      if (ptr == NULL
          && ((count = GC_objlink_free_pending (gcdata, size)) == 0
              || (ptr = GC_inner_core_malloc (gcdata, size, GC_dont_expand))
                   == NULL))
        {
#ifndef GC_NO_DLINKS
                                             ...
                                             int *pres)
{
  ...
  if ((objlinks_block_list = GC_core_malloc_with_gc (
         gcdata, count * sizeof (struct GC_objlink_s) + sizeof (GC_word), pres))
      == NULL)
    return 0;
  *(void **)objlinks_block_list = gcdata->objlinks_block_list;
  ...
  ((struct GC_objlink_s *)(objlinks_block_list
                           = (char *)objlinks_block_list + sizeof (GC_word)))
    ->obj
    = NULL;
  ...
  while (--count)
    {
      ((struct GC_objlink_s *)((GC_word)objlinks_block_list
                               + sizeof (struct GC_objlink_s)))
        ->next
        = objlinks_block_list;
      ((struct GC_objlink_s *)(objlinks_block_list
                               = (char *)objlinks_block_list
                                 + sizeof (struct GC_objlink_s)))
        ->obj
        = NULL;
    }
              ...
              if (
                ...
                gcdata->bytes_allocd)
                {
                  if (GC_dont_gc || gcdata->dataroots == NULL)
                    {
                      if ((GC_dont_gc == GC_NEVER_COLLECT
                           || gcdata->dataroots == NULL)
                          && (new_hroots = GC_alloc_hroots (
                                gcdata, gcdata->obj_htable.log2_size + 1,
                                &GC_nil_objlink))
                               != NULL)
                        {
                          ...
                          gcdata->obj_htable.free_list
                            = gcdata->obj_htable.marked_list;
              ...
              else
                {
                  ...
                  if (gcdata->bytes_allocd
                    ...
                    {
                      if (!GC_dont_expand
                          && gcdata->expanded_heapsize
                               <= gcdata->total_heapsize)
          ...
          if (obj != NULL)
            {
              ...
              if (!retry && gcdata->recycling && res > 0)
                {
                  res
                    = (int)GC_objlink_some_refill (gcdata, GC_LAZYREFILL_COUNT);
                  if (gcdata->obj_htable.free_list != NULL
                      && (res += (int)GC_objlink_free_pending (gcdata, 0) << 1)
                           > 1
                      && (res > 2
                          || GC_objlink_free_pending (
                               gcdata, gcdata->free_bytes
                                         + ((GC_word)1 << GC_LOG2_OFFIGNORE))
                               == 1))
                  ...
                  if (notifier_fn == 0 && !GC_dont_gc && !GC_finalize_on_demand
                    ...
                    {
                      if ((fn
                           = GC_fnlz_del_ready (gcdata, &objlink, &client_data))
                          != 0)
                        res = 1;
                      else
                  ...
                  )
                    {
                      ...
                      if (GC_verbose_gc)
                        fprintf (stdout,
                                 ...
                                 "KiB free of %lu KiB]" GC_NEW_LINE,
                                 GC_SIZE_TO_ULKB (gcdata->followscan_size),
                                 GC_SIZE_TO_ULKB (gcdata->marked_bytes
                                                  + gcdata->bytes_allocd
                                                  - gcdata->followscan_size),
                                 GC_SIZE_TO_ULKB (gcdata->free_bytes),
  ...
  if (low_addr != NULL)
    {
      ...
      if ((GC_word)low_addr < (GC_word)high_addr_plus_1)
        GC_roots_del_inside (gcdata, (GC_word)low_addr,
  ...
  if (displaced_pointer != NULL)
    {
      ...
      displaced_pointer
        = (objlink = GC_objlink_get (gcdata, displaced_pointer)) != NULL
              || (objlink = GC_objlink_refill_find (gcdata, displaced_pointer))
                   != NULL
            ? objlink->obj
            : NULL;
      GC_LEAVE (gcdata);
      ...
      if ((objlink = GC_objlink_get (gcdata, obj)) != NULL
          || (objlink = GC_objlink_refill_find (gcdata, obj)) != NULL)
        {
          if (objlink->obj == obj)
            {
              res = GC_dlink_add (
                gcdata, GC_HIDE_POINTER (link), objlink,
                (new_dlink = gcdata->dlink_htable.free_list) != NULL
                  ...
                  : ((GC_HASH_RESIZECOND (gcdata->dlink_htable.count,
                          ...
                          && (new_hroots = GC_alloc_hroots (
                                gcdata,
                                new_log2_size
                                = gcdata->dlink_htable.log2_size + 1,
                                &GC_nil_dlink))
                               != NULL
                        ? (GC_dlink_tblresize (gcdata, new_hroots,
                                               new_log2_size),
                           0)
                        : 0),
                     GC_core_malloc_with_gc (gcdata, sizeof (struct GC_dlink_s),
                                             &res)));
              obj = NULL;
            }
        }
                                                    ...
                                                    void **odata)
{
  ...
  GC_enter (&gcdata);
  if (obj != NULL
      && ((objlink = GC_objlink_get (gcdata, obj)) != NULL
          || (objlink = GC_objlink_refill_find (gcdata, obj)) != NULL))
    {
      if (objlink->obj == obj)
        {
          if (fn != 0)
            {
              if (GC_HASH_RESIZECOND (gcdata->fnlz_htable.count,
                  ...
                  && (new_hroots = GC_alloc_hroots (
                        gcdata,
                        new_log2_size = gcdata->fnlz_htable.log2_size + 1,
                        &GC_nil_fnlz))
                       != NULL)
                GC_fnlz_tblresize (gcdata, new_hroots, new_log2_size);
              if (gcdata->fnlz_htable.single_free == NULL)
                res
                  = (gcdata->fnlz_htable.single_free = GC_core_malloc_with_gc (
                       gcdata, sizeof (struct GC_fnlz_s), &res))
                        != NULL
                      ? 1
                      : -1;
            }
          obj = NULL;
          if ((fn = GC_fnlz_add_del (gcdata, objlink, fn, client_data, odata))
              != 0)
            {
              if (ofn != NULL)
                *ofn = fn;
