...
static object_t _with_real_op (vm_t vm, object_t ret, imm_object_t x,
                               imm_object_t y, num_opt_t opn)
{
  switch (y->attr.type)
    {
    case complex_inexact:
    case complex_exact:
      PANIC ("Complex number is not supported yet!\n");
    case real:
      {
        real_t a, b;
        ret->attr.type = real;
    ...
    case imm_int:
      {
        real_int_op_t op = (real_int_op_t)get_num_op (opn);
        real_t a;
        a.v = (uintptr_t)x->value;

        ret->value = (void *)op (a.f, (s32_t)y->value);
...
{
  rational_t r;
  s64_t upper = a.numerator + b * a.denominator * a.sign;
...
{
  rational_t r;
  s64_t diff = a.numerator - b * a.denominator * a.sign;
  r.denominator = a.denominator;
                                   ...
                                   immu_object_t y, num_op_t op)
{
  switch (y->attr.type)
    ...
    case ragional:
      {
        ...
        rat_rat_op_t op = (rat_rat_op_t)get_num_op (op);
        rational_t result = op ((rational_t)x->value, (rational_t)y->value);

        ret->value = (void *)result;
        ret->attr.type = rational;

        return ret;
      }
    case imm_int:
      {
        rat_int_op_t op = (rat_int_op_t)get_num_op (op);
        rational_t result = op ((rational_t)x->value, (s32_t)y->value);
    ...
    case real:
      {
        // TODO:
        // convert rational to real, then call _with_real_op
        PANIC ("Rational operation with real is not supported yet!\n");
      }
    default:
                         ...
                         immu_object_t y, num_op_t opn)
{
  switch (x->attr.type)
    ...
    case real:
      {
        return _with_real_op (vm, ret, x, y, opn);
      }
    case imm_int:
      {
        switch (y->attr.type)
          ...
          case real:
            {
              return _with_real_op (vm, ret, y, x, opn);
            }
          case imm_int:
            {
              int_int_op_t op = (int_int_op_t)get_num_op (opn, int_int);
              ret->value = (void *)op ((s32_t)x->value, (s32_t)y->value);
              ret->attr.type = imm_int;
              return ret;
            }
          default:
...
bool eq_with_int (imm_object_t x, imm_object_t y)
{
  switch (y->attr.type)
    {
    case imm_int:
...
bool eq_with_rag (imm_object_t x, imm_object_t y)
{
  switch (y->attr.type)
    {
    case imm_int:
    ...
    default:
      PANIC ("Type error, x type = %d\n", x->attr.type);
    }
}

bool eq_with_real (imm_object_t x, imm_object_t y)
{
  switch (y->attr.type)
    {
    case imm_int:
...
bool gt_with_int (imm_object_t x, imm_object_t y)
{
  switch (y->attr.type)
    {
    case imm_int:
    ...
    case complex_exact:
      PANIC ("Complex_inexact is not supported yet!\n");
    default:
      PANIC ("Type error, y type = %d\n", y->attr.type);
    }
}

bool gt_with_rag (imm_object_t x, imm_object_t y)
{
  switch (y->attr.type)
    {
    case imm_int:
    ...
    case rational:
      {
        rational_t a = (rational_t)x->value;
        rational_t b = (rational_t)y->value;
        s64_t left = a.numerator * b.denominator * a.sign;
        s64_t right = b.numerator * a.denominator * b.sign;
        return left > right;
      }
    case real:
      {
...
bool gt_with_real (imm_object_t x, imm_object_t y)
{
  switch (y->attr.type)
    {
    case imm_int:
...
bool _int_gt (object_t x, object_t y)
{
  ...
  static object_t _os_get_board_id (vm_t vm, object_t ret)
  {
    ...
    char *uid
      = (char *)GC_MALLOC (BOARD_ID_LEN); // last is \0, shall be included
    ret->value = (void *)uid;
  ...
  static object_t _os_device_configure (vm_t vm, object_t ret, object_t obj)
  {
    ...
    *ret = GLOBAL_REF (none_const);
    ...
    if (SUPERDEVICE_TYPE_GPIO_PIN == p->type)
      {
        gpio_pin_configure (p->dev, p->gpio_pin,
                                      ...
                                      object_t value)
  {
    ...
    int status = i2c_reg_write_byte (p->dev, (imm_int_t)dev_addr->value,
                                     ...
                                     object_t i2c_addr, object_t length)
  {
    ...
    int status
      = i2c_read (p->dev, rx_buf, len_list, (imm_int_t)i2c_addr->value);
    if (status != 0)
      {
        *ret = GLOBAL_REF (false_const);
        os_free (rx_buf);
                                      ...
                                      object_t i2c_addr, object_t lst)
  {
    ...
    int status
      = i2c_write (p->dev, tx_buf, len_list, (imm_int_t)i2c_addr->value);
                                           ...
                                           object_t i2c_addr, object_t length)
  {
    ...
    bv->attr.gc
      = PERMANENT_OBJ; // avoid unexpected collection by GC before done
    bv->vec = buf;
    bv->size = len_list;
...
#  ifndef SIMULATE
    os_printk ("i2c_read_bytevector (%s, 0x%02X, %d)\n",
               (const char *)dev->value, (imm_int_t)i2c_addr->value,
               (imm_int_t)length->value);
    *ret = GLOBAL_REF (false_const);
    return ret;
#  else
    struct super_device super_dev_0 = {0};
...
#  endif /* SIMULATE */
                                         ...
                                         object_t i2c_addr, object_t bv)
  {
    ...
    os_printk ("i2c_reg_write_bytevector (%s, 0x%02X, ",
               (const char *)dev->value, (imm_int_t)i2c_addr->value);
    object_printer (bv);
    os_printk (")\n");
