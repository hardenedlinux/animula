    ...
    case imm_int:
      {
        ...
        result = (int_int_t)op (x->value, y->value);
...
static rational_t rational_minus (rational_t a, rational_t b)
{
  rational_t r;
  s32_t diff = a.numerator * b.denominator * a.sign
    - b.numerator * a.denominator * b.sign;
  u32_t base = a.denominator * b.denominator;
...
{
  rational_t r;
  u32_t upper = a.numerator * b.numerator;
  u32_t base = a.denominator * b.denominator;
  r.numerator = upper & 0xFFFF;
  r.denominator = base & 0xFFFF;
...
static rational_t rational_div (rational_t a, rational_t b)
{
  ...
  s32_t upper = a.numerator * b.denominator;
  s32_t base = a.denominator * b.numerator;
  r.numerator = upper & 0xFFFF;
  r.denominator = base & 0xFFFF;
  r.sign = (a.sign ^ b.sign);
    ...
    case rational_neg:
      {
        ...
        rational_t result
          = (rat_rat_op_t)op ((rational_t)x->value, (rational_t)y->value);
    ...
    case imm_int:
      {
        ...
        rational_t result
          = (rat_int_op_t)op ((rational_t)x->value, (s32_t)y->value);
                         ...
                         immu_object_t y, void *op)
{
  switch (x->attr.type)
    ...
    case real:
      {
        return _with_real_op (vm, ret, x, y, op);
      }
    case imm_int:
      {
        switch (y->attr.type)
          ...
          case real:
            {
              return _with_real_op (vm, ret, y, x, op);
            }
          case imm_int:
            {
              ret->value = (int_int_op_t)op ((s32_t)x->value, (s32_t)y->value);
              ret->attr.type = imm_int;
              return ret;
            }
          default:
  ...
  static object_t _int_add (vm_t vm, object_t ret, object_t xx, object_t yy)
  {
    ...
    // rational add other type
             ...
             || (rational_neg == y->attr.type)
             || (rational_pos == y->attr.type))
      {
                                   ...
                                   object_t yy)
  {
    ...
    if (complex_inexact == x->attr.type || complex_inexact == y->attr.type)
    ...
    else if (real == x->attr.type || real == y->attr.type)
      {
        ...
        else if ((rational_pos == x->attr.type)
                 || (rational_neg == x->attr.type))
          {
        ...
        if (y->attr.type == x->attr.type)
          {
            if (x->value == y->value)
              {
                PANIC (
		       "No formalized rational number is equal to an integer!\n");
                return true;
              }
            else
              {
